local pljacka = false -- Indique si un vol est en cours
local opljackao = false -- Indique si le vol a déjà eu lieu

if Config.KoristiBlipove == true then -- Si l'utilisation des blips est activée
    Citizen.CreateThread(function ()
      for i, v in pairs(Config.Blipovi) do
          ...
          AddTextComponentString(v.Ime)  -- Définir le nom du blip
            ...
            if opljackao == false then
              ESX.TriggerServerCallback('GS_Atmrobbery:tabletChecker', function(ima)  
                if ima then
                    ...
                    local uspeo = lib.skillCheck({'easy', 'easy', {areaSize = 70, speedMultiplier = 1}, 'easy'}, {'z', 'q', 's', 'd'})
